...
//     // 1. Update URL instantly
//     window.history.replaceState(null, "", newUrl);

//     // 2. Trigger navigation → makes Suspense kick in
...
const JobFilters = ({ categories }: JobFiltersProps) => {
  ...
  // Hybrid update function: update URL instantly + trigger server navigation
  const updateParams = (key: string, value: string | null) => {
    const params = new URLSearchParams(searchParams.toString());
    if (!value || value === "" || value === "all") {
      params.delete(key);
    ...
    // 1️⃣ Update URL immediately
    window.history.replaceState(null, "", newUrl);

    // 2️⃣ Trigger server navigation to load new data (Suspense fallback will show)
    router.replace(newUrl, { scroll: false });
  };
