...
export default function ShareNews({ className, newsId, newsTitle }: ShareNewsProps) {
  const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || "https://www.doxiverse.com";
  const postUrl = `${baseUrl}/blogs/${newsId}`;
  const encodedUrl = encodeURIComponent(postUrl);
  const title = "Check out this blog post by doxiverse:";
  const encodedTitle = newsTitle ? encodeURIComponent(newsTitle) : title;
